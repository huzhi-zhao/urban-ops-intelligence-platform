...
- 🔴 **本仓库对公，私有笔记对私，依赖方向是单向的。** 私有知识库可以引用本仓库，
  **本仓库不得引用它**——不得出现私有文档的标题、路径、链接，也不得出现只在
  那边成立的上下文（私人往来、对话记录、未公开的第三方材料、个人排期）。
  判据一句话：**这条引用，一个只有本仓库访问权的人能不能验证？**
  不能就是泄漏，即使它只是一个标题。
  上一条管的是**内容**不进来，这条管的是**引用**也不能出去指过去。
  一篇文档若离开私有上下文就立不住，说明它的立论本来就该在仓库内部重建
  （例如改为引用 `docs/dev/requirements/` 里已有的条目），而不是加一条外链。
