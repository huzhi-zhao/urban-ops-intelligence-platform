...
Storage : reads  s3a://{bucket}/bronze/raw/SRC-DCP/borough_boundaries/data_static.ndjson.gz
          writes s3a://{bucket}/silver/borough_boundaries/

Trigger example (Airflow UI → Trigger DAG w/ Config):
    {"bucket": "nyc-uoip-prod"}
    or leave bucket empty to use S3_BUCKET_NAME env var.
"""
...
def _check_params(**context) -> str:
    ...
    logger.info(
        ...
        "source=s3a://%s/bronze/raw/SRC-DCP/borough_boundaries/data_static.ndjson",
        bucket, bucket,
    )
    return bucket
...
with DAG(
    ...
    description="Full-refresh: DCP borough boundaries, Bronze -> Silver Parquet (WKT geometry)",
