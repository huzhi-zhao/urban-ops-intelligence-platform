...
- **失败路径必须留下。** 被中断的跑批、走错的排查方向、事后被推翻的归因，
  连同**推翻它的证据**一起留在原处。"最后成功了"是信息量最低的一句话；
  "两次误判为 OOM、真因是 commit 阶段的 rename"才是下一个人需要的。
  已推翻的归因**不删**，追加更正，否则读者无法判断某条排查路径是没试过还是试过不通。

- **判据贴原始输出。** 命令 + 结果，不写"已验证"。
  一条可以重跑的命令等于把结论的有效期交给后人自己判断，
  一句"已验证"只能被信任或被重做一遍。

- **被否决的选项与其理由必须留下。** 否则下一个人会重新提一遍同一个方案，
  而且没有材料反驳。ADR 与 design doc 各自的这一节都是全篇价值最高的部分。

反过来，**留下不等于摊开**。证据可以压缩：一行结论 + 一条可重跑的命令
优于三段叙述。要删的是叙述，不是数字、不是命令、不是那次失败。
