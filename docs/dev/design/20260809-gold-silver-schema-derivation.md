...
## 8. 开放项（2026-08-12 已逐条定稿，O2/O6 除外——不阻塞 S1，按原定时间点处理）

| # | 问题 | 结论 | 定的时间点 |
|---|---|---|---|
| **O1** | `fact_service_request_zone_event` 存不存零格 | **满面板（1,298 行），不只存非零。** M1 训练需要显式的零——零请求是有效信号，不是缺失；只存非零会让下游一旦忘记补零就系统性训出偏差 | ✅ S1 定稿 |
| **O2** | `dim_service_type` / `dim_channel` 谁维护、怎么保证不漏新取值 | 种子表进 `config/`，配一个"新取值检测"断言。维护责任人待 S3 定 | S3（contract 要写值域），不阻塞本篇定稿 |
