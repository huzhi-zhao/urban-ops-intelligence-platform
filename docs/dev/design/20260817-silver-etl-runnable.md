...
- 全表 `COUNT(*)` 与 Bronze 18.4 M 的差额**必须能被 `silver/_rejects/service_request/`
  的行数解释**,对不上就是丢行。
- 冬季子集行数量级对齐契约的 `winter_subset_approx ≈ 275,282`（比例判据,不是精确值）。
- 探针复现:空间命中 **134,123 / 134,258**。对不上时**信探针** —— 管道里有一步口径不一致。
