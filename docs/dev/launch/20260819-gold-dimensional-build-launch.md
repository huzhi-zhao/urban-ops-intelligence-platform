...
把 14 张 Gold 表从零行填满：9 张维表 + 5 张描述性/直通事实表。
...
- [x] D6 空间命中率已复现 ✅（2026-08-20，§4.14）：管道 **133,291 / 133,426 =
      99.8988%**，探针同日 **134,146 / 134,281 = 99.90%**，台账 99.9%。
      🟢 **`outside every zone` = 135，三处完全相同**——这是衡量几何正确性的那个数。
      🟡 分母差 855（0.64%），归因于 Bronze 采集范围而非口径，见 O19
...
- [x] E1 DQ 基线 ✅（2026-08-20，§3.x）：14 张表逐张记行数 · 列数 · 各列空值率，
      量在同一次全量构建上（`l2-20260820T033431Z`，1,920 秒）。
      **全空列 0**；六列非零空值全部有语义解释且对得上已有门禁。
...
- [x] **E2 `dag_gold_build` 在 Airflow 容器里跑通了**（2026-08-20）。
  ...
  - [x] E2b `only=seeds` 15 秒全绿 → 全量 14 张表 **2,127 秒全绿**，
        行数与 2026-08-19 逐张相同
...
- [x] E3 O11 的 C6 修订文本已同步（2026-08-19），实际是**四处**不是三处：
      ...
      **提前于阶段 E 执行**：`sql/dml/README.md` 就在阶段 D 要写 5 份 fact DML 的
      ...
      Gold（Trino）改整表重建四步。
- [x] E4 `CHANGELOG.md` ✅ **不记录**（2026-08-20，判断而非遗漏）。
      该文件开头自陈**不是发布日志**：一条变更只有在「持有既有数据的人必须做
      点什么」时才进来。L2 **一处 schema 都没动**（17 张 DDL 与 2026-08-13
      冻结的契约一致），做的是把空表填满，不存在需要迁移的既有数据。
      最接近够格的是 `dim_snowfall_event.is_scheduling_era` 的边界
      `2015-12-01` → `2015-11-01`（§4.6），但那是 DML 修 bug，且改时表仍为零行。
      🔴 **往里塞不合规矩的条目，就是把这个文件慢慢变成 git log 的副本**——
      正是它开头警告的那件事。L3 若动 schema（S7 冻结）再记。
- [ ] E5 分支 push + PR
...
✅ **阶段 D 完成（2026-08-19）：14 张表全部建成、门禁全绿、连跑两次行数逐张相同。**
...
### 3.x DQ 基线（E1，2026-08-20）

量在**同一次全量构建**上（`run_id=l2-20260820T033431Z`，1,920 秒，14 张全绿，
行数与上表逐张相同）。取数入口：`make gold-dq`。

| 表 | 段 | 行数 | 列数 | 有空值的列 | 全空列 |
|---|---|---|---|---|---|
| `dim_winter_category` | seeds | 7 | 6 | 0 | 0 |
| `dim_channel` | seeds | 15 | 6 | 0 | 0 |
| `dim_recommendation_rules` | seeds | 6 | 6 | 0 | 0 |
| `dim_service_type` | dims | 3,516 | 6 | 2 | 0 |
| `dim_plow_zone` | dims | 25 | 10 | 1 | 0 |
| `dim_admin_label` | dims | 252 | 5 | 0 | 0 |
| `dim_snowfall_event` | dims | 99 | 15 | 0 | 0 |
| `dim_plow_event` | dims | 19 | 8 | 1 | 0 |
| `dim_region_crosswalk` | dims | 548 | 9 | 0 | 0 |
| `fact_plow_shift` | facts | 418 | 9 | 0 | 0 |
| `fact_parking_ban` | facts | 49 | 8 | 1 | 0 |
| `fact_event_zone_rank` | facts | 418 | 8 | 1 | 0 |
| `fact_service_request_zone_event` | facts | 13,068 | 8 | 0 | 0 |
| `fact_winter_request_daily_by_label` | facts | 141,377 | 7 | 0 | 0 |

**全空列 0 —— 没有任何一列是「声明了但没人产出」**（`accum_flag` 那类故障的形状）。

#### 逐列空值率（未列出的列空值率 0%）

| 表 | 列 | 空值数 | 空值率 | 🟢 语义解释（都能对上已有门禁） |
|---|---|---|---|---|
| `dim_service_type` | `winter_category` | 3,312 | 94.20% | 非空 **204** = 冬季子集的 type 数；非冬季为 NULL 是设计（DML 头注 §37） |
| `dim_service_type` | `priority_weight` | 3,286 | 93.46% | 非空 **230**。**与上一行的 204 不等是对的**：两条互不相干的匹配——冬季类别匹 `dim_winter_category.keyword_pattern`，承诺时限匹 P1/P2/P3 正则。一个 type 可以写着「P2」却不是冬季作业 |
| `dim_plow_zone` | `area_delta_pct` | 17 | 68.00% | 非空 **8** = `geometry_repaired = true` 的 8 个，只有修过的几何才有面积差 |
| `dim_plow_event` | `matched_snowfall_event_id` | 2 | 10.53% | = `is_aligned = false` 的 2 个（BO-3 的无降雪犁雪） |
| `fact_parking_ban` | `matched_plow_event_id` | 30 | 61.22% | = 门禁的 19 匹配 / 30 NULL，语义不是缺数据 |
| `fact_event_zone_rank` | `matched_snowfall_event_id` | 44 | 10.53% | 44 = **2 个未对齐事件 × 22 个分区**，与上面那条同源 |

🔴 **两条读这张基线时必须知道的**：

1. **最新一两天的数据天生偏薄。** 上游 311 发布滞后约一天（§4.13），
   `fact_winter_request_daily_by_label` 是唯一按日期展开的表，其最后一两天的
   行数会明显低于同星期几的水平。**这是稳态不是缺口**，不要当故障查。
2. **空值率不是恒定的。** 上面六条都是「匹配上了多少」的函数：上游新增一批
   带 P2 字样的 type，`priority_weight` 的空值率就会动。**基线是对照物不是门禁**
   ——真要拦，得像行数门禁那样写成显式期望值。
...
自己今天也只给 69.8%（≈906）——916 已失效，见 §4.9** · 空间命中 ✅ **已复现**
（管道 99.8988%，探针同日 99.90%，`outside` 三处同为 135，见 §4.14）
...
阶段 D 的 5 张事实表首跑，3 张全绿、两条 FAIL。查完之后**没有一条是表的问题**——
...
而同一时间 `airflow dags test` 跑得好好的——14 张表 35 分钟全绿。
...
#### ✅ 已查清并闭合（2026-08-20）——结论要改，不是「Silver 断了三天」

上面那句「Silver 增量实际断了三天」**说对了现象、说错了成因**。查下来是两件事叠加，
真正的新知识是第二件：

**① Silver 忠实反映了 Bronze，08-18 的 8 行不是 Silver 的问题。**
`manifest_2026-08-18.json` 的 `record_count` 就是 **8**，与 Silver 那天的 8 行完全对上。

**② 🔴 上游 Socrata 发布滞后约一天——此前没有任何地方记过这件事。**
同一天（2026-08-20 03:1x）直接问上游要逐日条数：

| 日期 | 上游此刻 | 采集当时写进 Bronze 的 |
|---|---|---|
| 08-16 | 1,109 | 1,109 |
| 08-17 | 3,100 | 3,098（差 2 行，晚到） |
| **08-18** | **3,006** | **8** |
| 08-19 | 10 | —— |

`manifest` 的 `fetch_timestamp` 是 `2026-08-19T05:00:03`，即 08-19 那次 ingest 抓
08-18 时，**上游确实只有 8 条**。08-19 此刻只有 10 条，是同一现象的当期版本。

**所以「最新那一天的 Bronze 必然是薄的」是稳态，不是故障。**
补厚靠的是 `dag_ingest_service_requests` 的 7 天回溯（`LOOKBACK_DAYS = 7`，
每次重写最近 7 个 Bronze 文件）——08-17 那 2 行的差额正是回溯平时在干的活。
**三次 failed run 打断的不是采集，是这一层自愈机制。**

#### 处置与实测结果

1. Bronze 侧 CLI 重抓 `[2026-08-12, 2026-08-20)`，8 天一次过：
   2,987 / 3,004 / 2,760 / 1,385 / 1,109 / 3,100 / **3,006** / **10**。
   08-13～08-16 与上表的 Silver 行数逐日相同，口径无第二个问题。
2. 触发 `dag_backfill_silver_service_request` 同窗口。Trino 侧复核：
   **08-18 = 3,006 · 08-19 = 10**，与 Bronze 逐日相同。O17 闭合。
3. `ONLY=facts` 重跑 Gold（14 分钟）：**五张事实表行数逐张相同**——
   418 / 49 / 418 / 13,068·2,178·1,298 / 141,377，门禁全绿。
   上面那条「推理但未实测」现在是实测：8 月中旬这三天既不落进任何降雪事件窗口
   （908 那条下界门禁实测仍是 908），也不进 F8 的年份集合（仍 18 个年份）。
4. 随后又跑了一次**全量**（1,920 秒，14 张全绿），因为 `dim_service_type` 与
   `dim_admin_label` 枚举全历史、理论上可能被这三天带进新取值。**实测没有**：
   3,516（anti-join 0）与 252 (15+237) 均未变。这样 DQ 基线的 14 张表来自
   同一次构建，口径干净。

#### 三条留给后面的

- 🔴 **DQ 基线（E1）里最新一两天的行数天生偏薄，这是稳态。** 写基线时必须标注，
  否则下次有人看到 10 行就会当缺口查一遍。
- 🟡 **`.env` 的坑要连着这条看**：`.env` 被改成宿主机视角 → 三次 run failed →
  7 天回溯停摆 → 一天的数据停在 8 行。中间隔了三层，**告警只报了最外面那层**
  （`TrinoConnectionError`），没有任何一处输出会说「有一天只有 8 行」。
- 🟡 `airflow dags list-runs` 在 Airflow 3 里**不吃 `-d`**，dag_id 是位置参数。


### 4.14 ✅ D6：空间命中率三处复现，一致的是那个该一致的数

| 来源 | 分子 | 分母 | 命中率 | `outside every zone` |
|---|---|---|---|---|
| 台账（2026-08-09 探针） | 134,123 | 134,258 | 99.9% | 135 |
| 探针（2026-08-20 重跑） | 134,146 | 134,281 | 99.90% | **135** |
| 管道（Silver + `dim_service_type`） | 133,291 | 133,426 | 99.8988% | **135** |

🟢 **`outside every zone` 三处同为 135。** 这是这次比对里最有信息量的一个数：
它衡量的是几何本身是否正确（含 8 个 `make_valid` 修复过的多边形），三处一致
说明 `spark/transforms/zone_assignment.py` 与探针的 `ZoneIndex` 数的是同一套几何。
命中率也对到小数点后两位。**D6 达成。**

管道侧取数（`geo_match_status` 三值天然把 `no_geo` 挡在分母外，正是 E0 当初
拒绝把三值塌成 NULL 的理由）：

```sql
SELECT s.geo_match_status, count(*)
FROM silver_service_request s
JOIN uoip_gold.dim_service_type d ON s."type" = d."type"
WHERE d.winter_category IS NOT NULL AND s.open_date_local >= DATE '2015-11-01'
GROUP BY 1
-- matched 133,291 · unmatched 135 · no_geo 39,127
```

`era_start = 2015-11-01` 取自探针 CLI 默认值，**不是照抄注释**（§4.6 的教训）。

#### 🟡 分母差 855（0.64%）：不是口径差异

先排除了最像的那个解释：探针的 `WINTER_TYPE_PATTERNS`（7 个模式）与
`config/seeds/winter_category.csv` 的 `keyword_pattern` **逐条相同**，
含那个把冬季占比从 1.50% 虚涨到 10.40% 的 `%ICE CONTROL%`。词表一致。

剩下的解释是**数据范围**：探针读实时上游的全历史，管道读 Silver，而
**Bronze 的采集范围有意不是全历史**——2016-08-01 起才全天，之前只采冬季。
`era_start` 2015-11-01 正好落在那条线之前 9 个月。**这条推理未实测**，记为 O19。

⚠️ 与 §4.9 的漂移是两回事：那条是同一探针在两个时点给出不同的数（上游回修
存档 → 事件边界重切），这条是**探针与管道在同一时点**读的数据范围不同。
...
- ✅ **O17 已关闭（2026-08-20，见 §4.13）。** 成因不是「Silver 断了三天」，而是
  **上游滞后一天 + 三次 failed run 打断了 7 天回溯的自愈**。Bronze 与 Silver 均已补齐
  （08-18 = 3,006 · 08-19 = 10），`ONLY=facts` 重跑实测五张事实表行数逐张相同。
  🔴 派生约束：**DQ 基线里最新一两天偏薄是稳态**，写基线时必须标注。
- 🟡 **O19（新，2026-08-20，见 §4.14）：D6 的分母差 855 未实测归因。**
  推理是 Bronze 采集范围（2016-08-01 前只采冬季）而非口径差异——词表已核对逐条相同。
  不影响 D6 结论（命中率与 `outside` = 135 三处一致），但 L3 若要在 2016-08 之前的
  窗口上训练 M1，这 855 行就是必须先查清的东西。
...
| 阶段 E（收口） | 🚧 进行中：E3 早已完成，**E2 已跑通**（2026-08-20，四个缺陷见 §4.10），**E1 DQ 基线已完成**（§3.x），余 E2c / E4 / E5 / E6 |
...
2. ~~阶段 D 的 5 份事实表 DML~~ ✅ 已写完并**跑通生产**（2026-08-19）：
   14 张表全部建成，改完两个门禁数字后全绿。数字在 §3，两条更正在 §4.9。
3. ~~D10 复跑~~ ✅ **已完成**：连跑两次行数逐张相同，第二趟全绿。
   R4 的 purge 在事实表上验证完毕。**阶段 D 关闭。**
4. **阶段 E 收口。进行中。** ✅ **E2 已完全跑通**（2026-08-20）：全量 14 张表在
   ...
   **下一个会话从这里开始，按顺序：**
   ...
   1. ~~修 O17~~ ✅ **已关闭（2026-08-20）**，见 §4.13。成因是上游滞后一天 +
      三次 failed run 打断了 7 天回溯；Bronze / Silver 均已补齐，
      `ONLY=facts` 重跑实测五张事实表行数逐张相同（418 / 49 / 418 /
      13,068·2,178·1,298 / 141,377，门禁全绿）。
   2. ~~`ONLY=facts` 对行数~~ ✅ 同上，已实测。
   3. ~~E1 DQ 基线~~ ✅ **已完成（2026-08-20）**，见 §3.x。14 张表的行数 /
      列数 / 逐列空值率量在同一次全量构建上（`l2-20260820T033431Z`，1,920 秒）。
      **全空列 0**，六列非零空值全部有语义解释且对得上已有门禁。
   4. ~~D6 空间命中率复现~~ ✅ **已完成（2026-08-20）**，见 §4.14。
      **余下：** E4 CHANGELOG · E5 PR。
...
- **compose 的 `--env-file` 指的是仓库根的 `.env`**，不是 `infra/docker/.env`
  ——后者不存在，写错了直接 `couldn't find env file`。
- **`airflow dags list-runs` 在 Airflow 3 里不吃 `-d`**，dag_id 是位置参数。
- **上游 311 发布滞后约一天**，所以最新那天的 Bronze / Silver 必然很薄
  （实测 08-19 只有 10 行）。这是稳态不是缺口，靠 7 天回溯补厚——见 §4.13。
