...
> **Status**: 阶段 0–4 已完成（2026-08-30 → 08-31，§17）· **阶段 5a 已验收（§18.8）** · **阶段 5b 已验收并收口（§19.7）· 19 张图全绿** · **L15 已闭合（§20）** · **开始**: 2026-08-27
>
...
| ~~L3~~ | ~~取 Grafana 三块面板的 SQL 反向进仓~~ **已撤销（2026-08-31，§19.7）**：那三块面板是**数据源连通性的验证件**，不是交付图，不进仓 | — |
...
| ~~L9~~ | CLAUDE.md 里 BO-3 的「必须再加滚动累积判据」写成了待办，**实际已交付**（`accum_flag` 8 个 true，§14.3） | ✅ **已闭合（2026-08-31，§22）**：CLAUDE.md 已改成完成态，并带上「不得说 4 次犁雪被救回」的保留 |
...
| ~~L15~~ | 🔴 `attribution_text` 把降雪量渲染成 `2.02E1`（科学计数法），面向读者的句子（§17.7） | ✅ **已闭合（2026-08-31，§20.3）**：代码已修、F7 已重建、句子实测 `26.2 cm` |
| ~~L16~~ | 🔴 CLAUDE.md 与 L3 launch §8 对禁语 ② 的**理由**写错了（把经验事实写成结构不可能，§17.5）。禁语本身不动，理由要改 | ✅ **已闭合（2026-08-31，§21）**：CLAUDE.md · L3 launch §4.15 与禁语表三处均已改 |
...
### 19.7 阶段 5b 验收结果（2026-08-31 实测）：19 张图全绿

`git pull` 后 lint 全绿、单测 **138 passed**，`make eda-run` **19 张全部执行成功**，
行数与 §19.4 预写表**逐张相符**：

| 图 | 预写 | 实测 |
|---|---|---|
| FIG-BO1-01 / 02 / 03 | 36 / 1 / 308 | **36 / 1 / 308** ✅ |
| FIG-BO2-01…05 | 22 / 22 / 418 / 22 / 49 | **同** ✅ |
| FIG-BO3-01 / 02 / 03 | 99 / 18 / 19 | **99 / 18 / 19** ✅ |
| FIG-BO4-01 / 02 / 03 | 109 / 25 / 7 | **同** ✅ |
| FIG-BO6-01 / 02 / 03 | 1,298 / 3 / ≤ 8 | **1,298 / 3 / 7** ✅ |
| FIG-BO8-01 / 02 | ≤ 70 / 4 | **63 / 4** ✅ |
...
§19.5 把它们当成「有图无 SQL 的孤儿」是**判断错了**。它们是当初用来验证
**Grafana 数据源是否连通**的验证件，验证完就没有别的用途——孤儿的定义是
「上了台却没有可追溯的 SQL」，而它们从来就没打算上台。

本阶段（Gold 层 EDA + BO 结论验证）到此结束，三个载体的图**从这 19 份 SQL 重新正式配**，
不从旧面板改。那块把两个版本叠成一根柱子的位移面板因此也不必修——
它会被 `fig_bo8_01_rank_displacement.sql` 直接取代，而后者本来就带 `GROUP BY model_version`。
...
### 20.4 实测：第一趟跑的是旧代码，而它 `all gates green`

🔴 **第一趟（`l2-20260831T154357Z`）节点上是 `5fedf7f`，修改在 `43e7108` 还没推。**
回显：七条门禁全绿、`748` 精确命中、`all gates green`——**而 F7 里仍然是
`2.02E1`**。判据只有一条：新门禁那一行**不在回显里**。

**一个不存在的门禁和一个通过的门禁长得一模一样。** 这是 §18.9（台账勾了不存在的
SQL）、§19.7（`grep "| true |"` 恒为 0）之后同一类缺陷的第三次，
三次的形状完全一致：**判据本身没跑，而输出是绿的**。
所以「新增门禁」的验收动作不是看颜色，是**先在回显里找到那一行**。

第二趟（`l2-20260831T154554Z`，`43e7108`）：

| 判据 | 实测 |
|---|---|
| `tables=1` | ✅ |
| 门禁条数 | **8**（第一趟 7） |
| `no attribution_text renders a number in scientific notation` | ✅ **0** |
| 行数 | **748**（= 2 × 374，两趟相同 → R4 的 purge 在 F7 上再次验证） |
| 人读抽样 | `"Zone H ranks high mainly on event severity (26.2 cm); …"` |

🟢 顺带一个可复用的读法：**两趟行数逐张相同**才是 purge 生效的证据。
`INSERT` 是追加，purge 失败的唯一表现就是第二趟变成 1,496，别的什么都不会响。

**L15 到此闭合**，`attribution_text` 可以投到载体上了。

---

## 21. L16：禁语 ② 的理由（2026-08-31）

禁语**本身不动**——`load_level` 依然不得跨 `score_weight_profile` 比较。
改的是**理由**，因为原来那条理由本身是错的，而错的方向最危险：
它把一个**经验事实**写成了**结构上的不可能**。

| | 原文 | 更正后 |
|---|---|---|
| CRITICAL 门槛（partial） | 75 | **52.5**（阈值按各自 ceiling 缩放，§17.5 八段边界实测） |
| 924 格 0 CRITICAL 的性质 | 「**永远不可能**」 | **经验事实**，最高 50.27，**差 2.23 分** |
| 禁语的依据 | 「尺子短三成」 | **同名不同尺**（两个 CRITICAL 不是同一个量）**+ 分布形状不同**（88.1% LOW vs 12.3%） |

🔴 **为什么这条值得单独修**：按原措辞，下一次重建冒出一个 partial 的 CRITICAL
时，**没有人会去查**——文档已经断言它不可能发生。一条写错理由的禁语，
比没有禁语更难纠正。

改动落三处：`CLAUDE.md` 四条禁语的第 ② 条 ·
`20260820-scoring-chain-and-m1-launch.md` **§4.15**（保留原文并标注被推翻）·
同篇 §8 的禁语表那一行。FIG-BO6-03 的 `must_not_say:` 头注在 §19 就已经是对的，
这次是让三份文档追上它。

---

## 22. L9：CLAUDE.md 的 BO-3 待办改成完成态（2026-08-31）

CLAUDE.md 「BO-3 必须在单日阈值之外再加一条滚动累积判据」自 2026-08-09 起是
待办体，而 §14.3 实测它**早已交付**：切分是「单日阈值 **或** 滚动累积」的并集，
`accum_flag = true` **8 个**、`NULL` 0。

🔴 **一条已经做完却还写着「必须做」的待办，代价不是噪音而是误导**——它让读的人
以为 `N = 99` 是旧口径下的数，从而怀疑一个正确的数字。

改成完成态时**一并写上保留**，因为那才是这条目前真实的未知：
⚠️ **不得说「那 4 次无降雪犁雪被这条判据救回来了」**——实测仍有 2 次
（2021-01-07 / 2026-02-26）`is_aligned = false`，8 个 accum 事件与那 4 次的关系
**没有量过**。把待办划掉而不带上这句，等于用一个过时的待办换一个乐观的错觉。
