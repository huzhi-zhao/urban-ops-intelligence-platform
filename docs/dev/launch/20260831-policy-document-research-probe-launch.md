...
- [ ] **给 Temi 发一条消息，两个问题一次问完**
      ① 20260827 那段录音里的那位是谁？
      ② 他能不能提供分级的原始文件？
      🔴 **一条消息发完，然后不等回复。** 它的延迟以天计，M 批不依赖它。
- [ ] 批准 design（本篇 §4 的门禁与 §3 的字段要求随之冻结）

出口：消息已发出 + design 状态为 Accepted。**H0 结束即可离开**。

### 1.2 M · 机器 · ≤ 3 小时（无人值守）

| 时刻 | 段 | 动作 | 结果 |
|---|---|---|---|
| | 预检 | 一个已知 URL 试 `WebFetch`（60 秒） | 待填 |
| | A（60 min） | 源清单 1–3 类**只定位不精读** | 待填 |
| | B（90 min） | 精读 + 摘逐字引文 + 存快照 | 待填 |
| | C（30 min） | 填 §3 台账 + 写裁决草案 + 自评 G1–G3/G5 | 待填 |
...
- [ ] Q1/Q2 采纳 / 放弃（放弃 = 20260828 outline 的可选插入页删掉）
- [ ] design §6 四个开放项各给一个状态
