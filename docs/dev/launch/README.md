...
- [20260831-policy-document-research-probe-launch.md](20260831-policy-document-research-probe-launch.md) ——
  **政策文件考古探针**（对应
  [design/20260831-policy-document-research-probe.md](../design/20260831-policy-document-research-probe.md)）。
  **未开始**：查证除雪街道分级 P1–P3 的成文依据，三个问题、3 小时时间盒。
  与前八篇不同，本次**不碰生产数据**——只读公开政策文件，产出是引文不是行数。
  提前开篇的理由与 L3 同类而与其余七篇都不同：**结论只在采集的那一刻存在**，
  市政网站会改版撤稿，所以 §3 的证据台账把「快照 + 检索日」列成与引文同等的必填项。
  §3 末尾那张检索轨迹表是为**未命中**准备的——查不到是合法交付，但必须可复核。
...
> **"上线后写"仍是默认，但八篇都提前开了篇**，理由各不相同：快照采集是不可逆
...
> 跨层对账与 BO-EDA 两篇的理由又不同：**方案与执行不在同一双手上**——
> 写方案的人拿不到线上环境，设计必须先写到可照着敲的程度。
> 考古探针那篇与 L3 同类：**上游文档会撤稿改版**，事后无法自证当初读到的是什么。
> 共同点仍是**等做完再写会丢掉过程信息**。
> 短的、一次做完的变更仍按上线后写。

> ✅ 十二篇命名一致，全部带 `YYYYMMDD-` 前缀（规则见下方「命名」一节，
