...
### Phase D · 退役与去 NYC ✅（2026-08-02 完成，批 0–1）
...
~~**优先级 0.5**：实测 `39ur-higg` 能否取到 plow zone 多边形。~~
✅ **已完成 2026-08-07**：82 个多边形 / 25 个 `plow_zone` 取值，
对全市 237,867 个地址点空间命中率 **100.0%**。
[ADR 0007](adr/0007-clearing-completion-time-source.md) §4.2 的降级方案确定不触发。

> 🔴 **但同一轮实测推翻了另一件事**：`tix9-r5tc` 是**排班计划表而非执行记录**，
...
```
0.40 × 预测服务请求量 + 0.30 × 排班顺位 + 0.30 × 天气严重度
```
...
> 更早的降雪事件没有排班记录，但**缺失不等于排在最后**——顺位因子在该区间标记
> NULL，不得填 0，否则会凭空造出 7 个冬季的假结论并传导到 Phase 5 的排序建议。
