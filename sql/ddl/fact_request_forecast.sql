...
CREATE TABLE IF NOT EXISTS fact_request_forecast (
    ...
    -- not_null
    -- note: SUM(request_count) across all 6 effective winter_category values for this (event_id,
    -- plow_zone) — see grain_relationship_to_f1 above.
    predicted_count DOUBLE,
    -- note: Seasonal-naive baseline for the same (event_id, plow_zone) — the "historical average
    -- request volume" method (§4.4 evaluation protocol). Stored on the same row per ADR 0010
    -- D5's "no model metric without a baseline, and never recomputed live" discipline (already
    -- applied to fact_recommendation.rank_baseline; this closes the same gap for M1's own MAE).
