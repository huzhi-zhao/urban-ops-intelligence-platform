...
-- Grain: (snowfall_event_id, plow_zone, winter_category)
...
-- (design doc §6.1) is defined and asserted at the (snowfall_event_id, plow_zone) grain
-- **within the scheduling-era subset only**, summed across categories — see the
-- relationships test below, not at this table's own row grain or its full 2178-cell extent.
-- Served by: BO-1
-- Primary key (informational only — Trino does not enforce PK/FK/UNIQUE): (snowfall_event_id,
-- plow_zone, winter_category)
-- unique: [['snowfall_event_id', 'plow_zone', 'winter_category']]
-- relationships:
...
--   COUNT(*) = 13068
--   COUNT(DISTINCT (snowfall_event_id, plow_zone)) = 2178
--   COUNT(DISTINCT (snowfall_event_id, plow_zone)) WHERE snowfall_event_id IN
--   (SELECT snowfall_event_id FROM dim_snowfall_event WHERE is_scheduling_era = true) = 1298
--   COUNT(*) FROM (SELECT snowfall_event_id, plow_zone FROM fact_service_request_zone_event WHERE
--   snowfall_event_id IN (SELECT snowfall_event_id FROM dim_snowfall_event
--   WHERE is_scheduling_era = true)
--   GROUP BY snowfall_event_id, plow_zone HAVING SUM(request_count) > 0) = 916
